...
# folder for runtime
target=/vol/home-vol2/se/nollerya/fuzzing/experiments/03-regex-7btime24hour-badger
...
do
  ...
  then
    ...
    cp "$src/config-server-berlin-7time24" "$current_target"
  fi
  ...
  then
    ...
    screen -S spf -d -m java -Xmx10240m -jar KelinciSPFRunner.jar config-server-berlin-7time24
  fi
