...
# folder for runtime
target=/vol/home-vol2/se/nollerya/fuzzing/experiments/03-regex-5imageextension-badger
...
do
  ...
  then
    ...
    cp "$src/config-server-berlin-5image" "$current_target"
  fi
  ...
  then
    ...
    screen -S spf -d -m java -Xmx10240m -jar KelinciSPFRunner.jar config-server-berlin-5image
  fi
