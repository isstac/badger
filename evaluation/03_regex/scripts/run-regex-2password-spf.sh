...
# folder for runtime
target=/vol/home-vol2/se/nollerya/fuzzing/experiments/03-regex-2password-spf
...
do
  ...
  cp "$src/config-server-berlin-2password-spf" "$current_target"
  ...
  screen -S spf -d -m java -Xmx10240m -jar KelinciSPFRunner.jar config-server-berlin-2password-spf
