...
# folder for runtime
target=/vol/home-vol2/se/nollerya/fuzzing/experiments/03-regex-8date-spf
...
do
  ...
  cp "$src/config-server-berlin-8date-spf" "$current_target"
  ...
  screen -S spf -d -m java -Xmx10240m -jar KelinciSPFRunner.jar config-server-berlin-8date-spf
