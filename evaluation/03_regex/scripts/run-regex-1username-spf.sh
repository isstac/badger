...
# folder for runtime
target=/vol/home-vol2/se/nollerya/fuzzing/experiments/03-regex-1username-spf
...
do
  ...
  cp "$src/config-server-berlin-1username-spf" "$current_target"
  ...
  screen -S spf -d -m java -Xmx10240m -jar KelinciSPFRunner.jar config-server-berlin-1username-spf
